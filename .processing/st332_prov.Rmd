...
```{r castsp w/o sport9}
castsp1 <-castsp[c(1:5,7)]
castsp1 <- as.data.frame(castsp1)
```

```{r mice design, include=FALSE}
castspmi <- mice(castsp1,m=5,maxit=50,meth='pmm')
```
```{r mice, output=FALSE, echo=FALSE}
castsp_comp<-complete(castspmi,1)
```
```{r }
castsp$bsport1 <- paste(castsp_comp$bsport)
castsp = subset(castsp,select= -c(bsport,bsportna))
```
...
```{r, output=TRUE, message=FALSE, warning=FALSE}
explanatory = c("age", "sex", 
  "height", "weight")
dependent = "sport9"
castsp %>% 
...
```{r }
library(corrplot)
corrcast<-castsp[c(1:4,6,7)]
#corrcast <- as.data.frame(corrcast)
#correlations <- cor(corrcast)
...
```{r }
castsp2<- castsp[,-5] #dropping sport9 variable

## changing sex values to 0/1
castsp2$sex<-as.numeric(castsp2$sex)
castsp2 <- castsp2 %>% mutate(sex=sex-1)
castsp2$sex<-factor(castsp2$sex)

castsp2$bsport1<-as.numeric(castsp2$bsport1)
```
```{r }
train <- castsp2[1:56,]
test <- castsp2[56:500,]
pred <- castsp2[500:565,]
```
...
```{r }
modellog1 <- glm(sport9na ~.,family=binomial(link='logit'),data=train)
summary(modellog1)

anova(modellog1, test="Chisq")

library(pscl) #for R-squared equivalent on glm's
pR2(modellog1) 


library(ROCR)
p <- predict(modellog1, test, type="response")
pr <- prediction(p, test$sport9na)
...
```{r model 1}
castsp2$sport9na<- factor(castsp2$sport9na)

spmodel1 <- glm(sport9na~age,data=castsp2,family="binomial")

spmodel2<- glm(sport9na~age+factor(sex)+weight+height+bsport1, data=castsp2,family="binomial")

spmodel3 <- glm(sport9na~(factor(sex)+weight+height+bsport1)*
                (factor(sex)+weight+height+bsport1),data=castsp2,family="binomial")

spmodel4 <- glm(sport9na~factor(sex)+weight+height+bsport1, data=castsp2,family="binomial")

spmodel5 <- glm(sport9na~bsport1*age, data=castsp2,family="binomial")
```
...
cast$sex<-as.numeric(cast$sex)
cast <- cast %>% mutate(sex=sex-1)
cast$sex<-factor(cast$sex)

cast<- cast %>% mutate(yscore_miss=Yscore-Yscore)

cast <- dummy_cols(cast, select_columns = "yscore_miss", remove_first_dummy=TRUE)

cast <- cast[,-16]
cast$bsport<-castsp2$bsport1
```

```{r }
modely1<-glm(yscore_miss_NA~bsport+age,data=cast,family="binomial")

tidy(modely1)
```

```{r }
casty <- read.csv("CAST2miss.csv")
casty <-as.data.frame(casty)
cast_Y <- casty[,c(1:9,15)]
cast_Y <- na.omit(cast_Y)
```
